  ...
  let hoverRequestDefinition = vscode.languages.registerHoverProvider(
    { pattern: '**/**.conf' },
    {
      provideHover(document, position, token) {
        ...
        outputChannel.appendLine('you just hover on "request_definition"');
        const range = document.getWordRangeAtPosition(position);
        const word = document.getText(range);

        if (word == 'request_definition') {
          console.log('show hover for "request_definition"');
          outputChannel.appendLine('show hover for "request_definition"');
          return {
            contents: [
        ...
        if (word == 'policy_definition') {
          ...
          outputChannel.appendLine('show hover for "policy_definition"');
          return {
            contents: [
